...
export default function VargehMulti(){
  ...
  const syncFromCloud=useCallback(async()=>{
    ...
    try{
      ...
      if(rec?.animals){
        // فقط اگر داده واقعاً تغییر کرده setAnimals را صدا بزن
  ...
  return(
    <div style={{fontFamily:"'Vazirmatn',sans-serif",direction:"rtl",background:C.bg,minHeight:"100vh",color:C.text,fontSize:14,paddingBottom:62}}>
        ...
        return(
          <div style={{padding:14,maxWidth:960,margin:"0 auto"}}>
            ...
            {dtab==="info"&&<div style={cs}><div style={g3}>
              {[["کد",a.id],["نوع",a.type],["ژنوتیپ",a.genotype],["تولد",a.birthDate],["سن",a.estimatedAge],["وزن ورود",a.entryWeight?`${toP(a.entryWeight)} کیلو`:null],["کد پدر",a.fatherCode||"—"],["کد مادر",a.motherCode||"—"],["وضعیت",a.status]].map(([l,v])=>(
      ...
      {addModal&&addModal!=="animal"&&sel&&<SubForm type={addModal} aid={sel.id} onClose={()=>{setAddModal(null);setEditRecord(null);resetForm({});}} animals={animals} saveAnimals={saveAnimals} toast_={toast_} editRecord={editRecord} formData={formData} updateForm={updateForm}/>}
      {editAnimalModal&&sel&&<EditAnimal onClose={()=>setEditAnimalModal(false)} animal={sel} animals={animals} saveAnimals={saveAnimals} toast_={toast_}/>}
