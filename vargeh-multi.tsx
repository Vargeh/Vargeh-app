...
const normDate=s=>{if(!s)return s;return s.replace(/[۰-۹]/g,d=>String("۰۱۲۳۴۵۶۷۸۹".indexOf(d))).replace(/[٠-٩]/g,d=>String("٠١٢٣٤٥٦٧٨٩".indexOf(d)));};
const isDate=d=>/^\d{4}\/\d{2}\/\d{2}$/.test(d);
const isDateValid=d=>{if(!isDate(d))return false;const[y,m,day]=d.split("/").map(Number);if(m<1||m>12)return false;if(day<1||day>sDim(m))return false;if(y<1300||y>1500)return false;return true;};
...
const ageInMonths=a=>{if(isDate(a.birthDate)){const df=daysFrom(a.birthDate);if(df===null||df>0)return null;// تاریخ آینده یا نامعتبر
const days=Math.abs(df);return Math.floor(days/30);}if(a.estimatedAge){const n=parseFloat(a.estimatedAge);if(!isNaN(n))return a.estimatedAge.includes("ماه")?n:n*12;}return null;};
...
function SubForm({type,aid,onClose,animals,saveAnimals,toast_,editRecord=null,formData={},updateForm}){
  ...
  const fd=formData;
  const sv=(k,v)=>{const df=(dateFields[type]||[]);updateForm(k,df.includes(k)?normDate(v):v);};
  ...
  // محاسبه تاریخ زایمان — هر بار که matingDate تغییر کند
  useEffect(()=>{
    if(type==="repro"&&fd.matingDate&&isDateValid(fd.matingDate)){
      const exp=addDays(fd.matingDate,GD);
      if(exp)updateForm("expDate",exp);
    }
  },[fd.matingDate]);

  const F=(l,k,opts,ph,tp)=>{
    const isDateField=(dateFields[type]||[]).includes(k);
    const hasErr=isDateField&&fd[k]&&!isDateValid(fd[k]);
    const borderColor=hasErr?C.danger:C.border;
    return(
    <div style={{marginBottom:10}}>
      <label style={lbl2}>{l}{hasErr&&<span style={{color:C.danger,marginRight:6,fontSize:10}}>فرمت اشتباه</span>}</label>
      {opts?<select value={fd[k]||""} onChange={e=>sv(k,e.target.value)} style={{...inpS2,border:`1px solid ${borderColor}`}}><option value="">—</option>{opts.map(o=><option key={o}>{o}</option>)}</select>
      :<input type={tp||"text"} value={fd[k]||""} placeholder={ph||""} onChange={e=>sv(k,e.target.value)} style={{...inpS2,border:`1px solid ${borderColor}`}}/>}
    </div>);
  };
  ...
  // فیلدهای تاریخ هر نوع فرم
  const dateFields={vaccine:["lastDate","nextDate"],weight:["date"],treat:["date"],repro:["matingDate","birthDate"],cost:["date"],rev:["date"],note:["date"]};
  const hasDateErr=(dateFields[type]||[]).some(k=>fd[k]&&!isDateValid(fd[k]));

  const save=()=>{
    if(hasDateErr){toast_("⚠️ فرمت تاریخ اشتباه است (مثال: ۱۴۰۵/۰۳/۱۵)","err");return;}
    if(type==="vaccine"&&!fd.name){toast_("⚠️ نوع واکسن را انتخاب کنید","err");return;}
    if(type==="vaccine"&&!fd.lastDate){toast_("⚠️ تاریخ تزریق را وارد کنید","err");return;}
    if(type==="vaccine"&&!fd.nextDate){toast_("⚠️ تاریخ تکرار بعدی را وارد کنید — برای هشدار خودکار لازم است","err");return;}
    if(type==="weight"&&fd.weight){const w=+fd.weight;if(isNaN(w)||w<5||w>200){toast_("⚠️ وزن باید بین ۵ تا ۲۰۰ کیلو باشد","err");return;}}
    if((type==="cost"||type==="rev")&&fd.amount){const a=+fd.amount;if(isNaN(a)||a<=0){toast_("⚠️ مبلغ باید بیشتر از صفر باشد","err");return;}}
    if(type==="repro"&&fd.lambCount){const lc=+fd.lambCount;if(isNaN(lc)||lc<0||lc>6){toast_("⚠️ تعداد بره باید بین ۰ تا ۶ باشد","err");return;}}
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:800,padding:14}}>
      <div style={{...cs2,width:"100%",maxWidth:420,maxHeight:"88vh",overflowY:"auto"}}>
        ...
        {type==="weight"&&<>{F("تاریخ","date",null,"۱۴۰۳/۰۹/۰۱")}{(()=>{const w=+fd.weight;const wErr=fd.weight&&(isNaN(w)||w<5||w>200);return(<div style={{marginBottom:10}}><label style={lbl2}>وزن (کیلو){wErr&&<span style={{color:C.danger,marginRight:6,fontSize:10}}>باید ۵–۲۰۰ باشد</span>}</label><input type="number" value={fd.weight||""} onChange={e=>sv("weight",e.target.value)} style={{...inpS2,border:`1px solid ${wErr?C.danger:C.border}`}}/></div>);})()}{F("وضعیت","physStatus",["ورود","نرمال","آبستن","شیردهی","بیمار"])}{F("یادداشت","note")}</>}
        ...
        {type==="cost"&&<>
          ...
          {(()=>{const aErr=fd.amount&&(isNaN(+fd.amount)||+fd.amount<=0);return(<div style={{marginBottom:10}}><label style={lbl2}>مبلغ (ریال){aErr&&<span style={{color:C.danger,marginRight:6,fontSize:10}}>باید بیشتر از صفر باشد</span>}</label><input type="number" value={fd.amount||""} onChange={e=>sv("amount",e.target.value)} placeholder="مثال: 5000000" style={{...inpS2,border:`1px solid ${aErr?C.danger:C.border}`}}/>{fd.amount&&!aErr&&<div style={{fontSize:11,color:C.gold,marginTop:4,fontWeight:600}}>= {fmtRial(+fd.amount)}</div>}</div>);})()}
          {F("توضیح","note")}
        </>}
        {type==="rev"&&<>
          {F("تاریخ","date")}{F("دسته","category",["فروش بره","فروش میش","فروش قوچ","فروش پشم","سایر"])}
          {(()=>{const aErr=fd.amount&&(isNaN(+fd.amount)||+fd.amount<=0);return(<div style={{marginBottom:10}}><label style={lbl2}>مبلغ (ریال){aErr&&<span style={{color:C.danger,marginRight:6,fontSize:10}}>باید بیشتر از صفر باشد</span>}</label><input type="number" value={fd.amount||""} onChange={e=>sv("amount",e.target.value)} placeholder="مثال: 18000000" style={{...inpS2,border:`1px solid ${aErr?C.danger:C.border}`}}/>{fd.amount&&!aErr&&<div style={{fontSize:11,color:C.gold,marginTop:4,fontWeight:600}}>= {fmtRial(+fd.amount)}</div>}</div>);})()}
          {F("توضیح","note")}
        </>}
...
function AddAnimal({onClose,animals,saveAnimals,toast_,formData={},updateForm,resetForm}){
  ...
  const sv=(k,v)=>updateForm(k,(k==="birthDate"||k==="entryDate")?normDate(v):v);
  const inpS2={background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontFamily:"inherit",fontSize:14,width:"100%",outline:"none",boxSizing:"border-box"};
  const lbl2={fontSize:11,color:C.textDim,marginBottom:4,display:"block",fontWeight:600};
  const g22={display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:12};
  const addDateFields=["birthDate","entryDate"];
  const F=(l,k,opts,ph,tp)=>{
    const isDF=addDateFields.includes(k);
    const hasErr=isDF&&fd[k]&&(!isDateValid(fd[k])||daysFrom(fd[k])>0);
    const bc=hasErr?C.danger:C.border;
    return(<div><label style={lbl2}>{l}{hasErr&&<span style={{color:C.danger,marginRight:6,fontSize:10}}>{fd[k]&&daysFrom(fd[k])>0?"تاریخ آینده":"فرمت اشتباه"}</span>}</label>
      {opts
        ?<select value={fd[k]||""} onChange={e=>sv(k,e.target.value)} style={{...inpS2,border:`1px solid ${bc}`}}>
          <option value="">—</option>
          {opts.map(o=><option key={o}>{o}</option>)}
        </select>
        :<input type={tp||"text"} value={fd[k]||""} placeholder={ph||""} onChange={e=>sv(k,e.target.value)} style={{...inpS2,border:`1px solid ${bc}`}}/>
      }
    </div>);
  };
  const save=()=>{
    if(!fd.id){toast_("⚠️ کد داخلی الزامی است","err");return;}
    if(animals.find(a=>a.id===fd.id)){toast_("⚠️ این کد قبلاً ثبت شده","err");return;}
    if(fd.birthDate&&!isDateValid(fd.birthDate)){toast_("⚠️ فرمت تاریخ تولد اشتباه است","err");return;}
    if(fd.birthDate&&daysFrom(fd.birthDate)>0){toast_("⚠️ تاریخ تولد نمی‌تواند در آینده باشد","err");return;}
    if(fd.entryDate&&!isDateValid(fd.entryDate)){toast_("⚠️ فرمت تاریخ ورود اشتباه است","err");return;}
    if(fd.entryDate&&daysFrom(fd.entryDate)>0){toast_("⚠️ تاریخ ورود نمی‌تواند در آینده باشد","err");return;}
    if(fd.entryWeight&&(isNaN(+fd.entryWeight)||+fd.entryWeight<5||+fd.entryWeight>200)){toast_("⚠️ وزن ورود باید بین ۵ تا ۲۰۰ کیلو باشد","err");return;}
    if(fd.purchasePrice&&(isNaN(+fd.purchasePrice)||+fd.purchasePrice<=0)){toast_("⚠️ قیمت خرید باید بیشتر از صفر باشد","err");return;}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:900,padding:14}}>
      <div style={{...cs2,width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={g22}>
          ...
          {isDate(fd.birthDate)?(<div><label style={lbl2}>سن (محاسبه‌شده)</label><input readOnly value={calcAge(fd)} style={{...inpS2,color:C.accent,cursor:"default"}}/></div>):F("سن تخمینی","estimatedAge",null,"۲ سال")}
...
function EditAnimal({onClose,animal,animals,saveAnimals,toast_}){
  ...
  const sv=(k,v)=>setFd(p=>({...p,[k]:(k==="birthDate"||k==="entryDate")?normDate(v):v}));
  const inpS2={background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontFamily:"inherit",fontSize:14,width:"100%",outline:"none",boxSizing:"border-box"};
  const lbl2={fontSize:11,color:C.textDim,marginBottom:4,display:"block",fontWeight:600};
  const g22={display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:12};
  const addDateFields=["birthDate","entryDate"];
  ...
  const editDateErr=[fd.birthDate,fd.entryDate].some(d=>d&&!isDateValid(d));
  const editFutureErr=fd.birthDate&&isDateValid(fd.birthDate)&&daysFrom(fd.birthDate)>0;
  const editWeightErr=fd.entryWeight&&(isNaN(+fd.entryWeight)||+fd.entryWeight<5||+fd.entryWeight>200);
  const save=()=>{
    if(editDateErr){toast_("⚠️ فرمت تاریخ اشتباه است (مثال: ۱۴۰۵/۰۳/۱۵)","err");return;}
    if(editFutureErr){toast_("⚠️ تاریخ تولد نمی‌تواند در آینده باشد","err");return;}
    if(editWeightErr){toast_("⚠️ وزن ورود باید بین ۵ تا ۲۰۰ کیلو باشد","err");return;}
...
export default function VargehMulti(){
  ...
  const[delSub_,setDelSub_]=useState(null);// {aid,field,rid,label}
  const[toast,setToast]=useState(null);
  const[editName,setEditName]=useState(null);
  const[editAnimalModal,setEditAnimalModal]=useState(0);
  ...
  const syncErrRef=useRef(null);
  const setSyncErrTimed=(msg)=>{setSyncErr(msg);clearTimeout(syncErrRef.current);if(msg)syncErrRef.current=setTimeout(()=>setSyncErr(""),8000);};
  ...
  const syncFromCloud=useCallback(async()=>{
    ...
    catch{setSyncErrTimed("خطا در دریافت");}
  ...
  const saveAnimals=useCallback(async(newA)=>{
    ...
    if(user){try{setSyncing(true);setSyncErr("");await writeBin(newA,user.name);setLastSync(new Date());}catch{setSyncErrTimed("خطا در ذخیره ابری");}finally{setSyncing(false);}}
  },[user,writeBin]);
  ...
  const confirmDelSub=(aid,field,rid,label)=>setDelSub_({aid,field,rid,label});
  const doDelSub=()=>{if(!delSub_)return;delSub(delSub_.aid,delSub_.field,delSub_.rid);setDelSub_(null);};
  ...
  return(
    <div style={{fontFamily:"'Vazirmatn',sans-serif",direction:"rtl",background:C.bg,minHeight:"100vh",color:C.text,fontSize:14,paddingBottom:62}}>
      ...
      {page==="cal"&&(
        <div style={{padding:14,maxWidth:960,margin:"0 auto"}}>
          <div style={{fontWeight:800,fontSize:17,marginBottom:14,color:C.accent}}>🗓️ تقویم گله</div>
          {(()=>{
            // واکسن‌های ۳۰ روز آینده
            const vaccCal=animals.filter(a=>a.status==="در گله").flatMap(a=>(a.vaccines||[]).filter(v=>{const d=daysFrom(v.nextDate);return d!==null&&d<=30;}).map(v=>({aid:a.id,aname:a.name||a.id,label:v.name,dl:daysFrom(v.nextDate),type:"vacc"})));
            // آماده جفت‌گیری — بیش از ۹۰ روز از آخرین زایمان و آبستن نیست
            const matingReady=animals.filter(a=>a.status==="در گله"&&a.type==="میش"&&!isPregnant(a)).filter(a=>{const d=daysSinceLastBirth(a);return d!==null&&d>=90;}).map(a=>{const d=daysSinceLastBirth(a);return{aid:a.id,aname:a.name||a.id,label:d>=100?"تأخیر در جفت‌گیری":"آماده جفت‌گیری",dl:0,type:"mate",days:d,urgent:d>=100};});
            // وزن‌کشی دوره‌ای — آخرین وزن‌کشی بیش از ۳۰ روز پیش
            const weightDue=animals.filter(a=>a.status==="در گله").filter(a=>{const ws=a.weights||[];if(!ws.length)return true;const last=ws[ws.length-1];const d=daysFrom(last.date);return d!==null&&d<-30;}).map(a=>{const ws=a.weights||[];const last=ws.length?ws[ws.length-1]:null;return{aid:a.id,aname:a.name||a.id,label:"وزن‌کشی معوق",dl:0,type:"weight",daysSince:last?Math.abs(daysFrom(last.date)||0):null};});
            return(<>
              {vaccCal.length>0&&<div style={{...cs,marginBottom:12,borderColor:C.info+"66"}}>
                <div style={{fontWeight:700,color:C.info,marginBottom:8}}>💉 واکسن‌های پیش‌رو</div>
                {vaccCal.sort((a,b)=>(a.dl||999)-(b.dl||999)).map((v,i)=><div key={i} onClick={()=>{setSelId(v.aid);setPage("detail");}} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}>
                  <span style={{fontWeight:600}}>{v.aname} — {v.label}</span>
                  <Bdg l={v.dl<=0?"🔴 واکسن معوق":v.dl<=7?`⚠️ ${toP(v.dl)} روز تا تزریق`:`${toP(v.dl)} روز تا تزریق`} c={v.dl<=0?C.danger:v.dl<=7?C.warn:C.info} sm/>
                </div>)}
              </div>}
              {matingReady.length>0&&<div style={{...cs,marginBottom:12,borderColor:matingReady.some(a=>a.urgent)?C.danger+"66":C.gold+"66"}}>
                <div style={{fontWeight:700,color:matingReady.some(a=>a.urgent)?C.danger:C.gold,marginBottom:8}}>🐑 آماده جفت‌گیری</div>
                {matingReady.map((a,i)=><div key={i} onClick={()=>{setSelId(a.aid);setPage("detail");}} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}>
                  <span style={{fontWeight:600}}>{a.aname}</span>
                  <Bdg l={a.urgent?`🔴 ${toP(a.days)} روز تأخیر`:`${toP(a.days)} روز از زایمان`} c={a.urgent?C.danger:C.gold} sm/>
                </div>)}
              </div>}
              {weightDue.length>0&&<div style={{...cs,marginBottom:12,borderColor:C.purple+"66"}}>
                <div style={{fontWeight:700,color:C.purple,marginBottom:8}}>⚖️ وزن‌کشی معوق</div>
                {weightDue.map((a,i)=><div key={i} onClick={()=>{setSelId(a.aid);setPage("detail");}} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}>
                  <span style={{fontWeight:600}}>{a.aname}</span>
                  <Bdg l={a.daysSince?`${toP(a.daysSince)} روز پیش`:"بدون سابقه"} c={C.purple} sm/>
                </div>)}
              </div>}
            </>);
          })()}
          <div style={{fontWeight:700,fontSize:14,marginBottom:10,color:C.gold}}>🗓️ زایمان‌های پیش‌رو</div>
          {birthCal.length===0&&<div style={{...cs,textAlign:"center",padding:40,color:C.textDim}}>میش آبستنی ثبت نشده</div>}
          {birthCal.map(e=>{
            const dlColor=e.dl===null?C.textDim:e.dl<=0?C.danger:e.dl<=7?C.danger:C.gold;
            const dlText=e.dl===null?"—":e.dl===0?"🔴 زایمان امروز":e.dl<0?`🔴 ${toP(Math.abs(e.dl))} روز تأخیر`:e.dl<=7?`⚠️ ${toP(e.dl)} روز تا زایمان`:`${toP(e.dl)} روز تا زایمان`;
            return(
            <div key={e.aid+e.s} onClick={()=>{setSelId(e.aid);setPage("detail");}} style={{...cs,marginBottom:10,cursor:"pointer",borderColor:e.dl!==null&&e.dl<=7?C.danger+"66":C.gold+"33"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><div style={{fontWeight:800,fontSize:14,color:C.accent}}>{e.aname}</div><div style={{fontSize:12,color:C.textMid}}>شکم {toP(e.s)} · {e.gen}</div></div>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:e.dl===0||e.dl<0?14:18,fontWeight:800,color:dlColor}}>{dlText}</div>
                  <div style={{fontSize:11,color:C.textDim,marginTop:2}}>{e.date}</div>
                </div>
              </div>
            </div>);
          })}
            ...
            <div style={{...cs,marginBottom:12,borderColor:C.accent+"33"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
                <div style={{display:"flex",gap:12,alignItems:"center"}}>
                  ...
                  <div>
                    <button onClick={()=>setEditAnimalModal(Date.now())} style={{...btnP,padding:"4px 10px",fontSize:11,marginBottom:6}}>✏️ ویرایش مشخصات</button>
                    {editName?.id===a.id?(
              ...
              {(a.vaccines||[]).map(v=>(<div key={v.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    ...
                    <button onClick={()=>confirmDelSub(a.id,"vaccines",v.id,`واکسن ${v.name}`)} style={btnDel}>🗑</button>
                  </div>
                </div>
              ...
              {(a.reproductions||[]).map(r=>(<div key={r.id} style={{...cs,borderColor:r.result==="در انتظار"?C.gold+"44":C.border}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                  ...
                  <div style={{display:"flex",gap:4,alignItems:"center"}}>
                    ...
                    <button onClick={()=>confirmDelSub(a.id,"reproductions",r.id,`زایمان شکم ${toP(r.season)}`)} style={btnDel}>🗑</button>
                  </div>
                </div>
                <div style={g3}>
            ...
            {dtab==="weight"&&<div style={cs}>
              ...
              {(a.weights||[]).map(w=>(<div key={w.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                ...
                <div style={{display:"flex",gap:4}}>
                  ...
                  <button onClick={()=>confirmDelSub(a.id,"weights",w.id,`وزن‌کشی ${w.date}`)} style={btnDel}>🗑</button>
                </div>
              </div>))}
            </div>}
              ...
              {(a.treatments||[]).map(t=>(<div key={t.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    ...
                    <button onClick={()=>confirmDelSub(a.id,"treatments",t.id,`درمان ${t.date}`)} style={btnDel}>🗑</button>
                  </div>
                </div>
            ...
            {dtab==="cost"&&<div style={cs}>
              ...
              {(a.costs||[]).map(c=>(<div key={c.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${C.border}`,alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:6,alignItems:"center"}}>
                  ...
                  <button onClick={()=>confirmDelSub(a.id,"costs",c.id,`هزینه ${c.category}`)} style={btnDel}>🗑</button>
                </div>
              </div>))}
            ...
            {dtab==="rev"&&<div style={cs}>
              ...
              {(a.revenues||[]).map(r=>(<div key={r.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${C.border}`,alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:6,alignItems:"center"}}>
                  ...
                  <button onClick={()=>confirmDelSub(a.id,"revenues",r.id,`درآمد ${r.category}`)} style={btnDel}>🗑</button>
                </div>
              </div>))}
              ...
              {(a.quickNotes||[]).slice().reverse().map(n=>(<div key={n.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  ...
                  <div style={{display:"flex",gap:4}}>
                    ...
                    <button onClick={()=>confirmDelSub(a.id,"quickNotes",n.id,"یادداشت")} style={btnDel}>🗑</button>
                  </div>
                </div>
              </div>))}
            </div>}
            ...
            {analysisTab==="vacc"&&<div>
              {overdueV.length>0&&<div style={{...cs,marginBottom:12,borderColor:C.danger+"66"}}>
                ...
                {overdueV.map((v,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
                  ...
                  <Bdg l={v.dl<=0?"🔴 واکسن معوق":v.dl<=7?`⚠️ ${toP(v.dl)} روز تا تزریق`:`${toP(v.dl)} روز تا تزریق`} c={v.dl<=0?C.danger:C.warn} sm/>
                </div>)}
              </div>}
              {(()=>{
                const noVaccAnimals=animals.filter(a=>a.status==="در گله"&&(!(a.vaccines||[]).length||(a.vaccines||[]).every(v=>!v.nextDate)));
                if(noVaccAnimals.length===0)return null;
                return(<div style={{...cs,marginBottom:12,borderColor:C.warn+"66"}}>
                  <div style={{fontWeight:700,color:C.warn,marginBottom:8}}>⚠️ بدون برنامه واکسیناسیون</div>
                  {noVaccAnimals.map((a,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.border}`}}>
                    <span style={{fontWeight:600}}>{a.name||a.id}</span>
                    <Bdg l="بدون برنامه" c={C.warn} sm/>
                  </div>)}
                </div>);
              })()}
              {vaccPlan.map((v,i)=>(
      ...
      {editAnimalModal>0&&sel&&<EditAnimal key={sel.id+"_"+editAnimalModal} onClose={()=>setEditAnimalModal(0)} animal={sel} animals={animals} saveAnimals={saveAnimals} toast_={toast_}/>}

      {delSub_&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:950,padding:16}}>
          <div style={{background:C.card,border:`1px solid ${C.danger}55`,borderRadius:12,padding:16,maxWidth:280,width:"100%",textAlign:"center"}}>
            <div style={{fontSize:32,marginBottom:8}}>🗑️</div>
            <div style={{fontWeight:700,fontSize:14,marginBottom:6}}>حذف {delSub_.label}؟</div>
            <div style={{color:C.textDim,fontSize:11,marginBottom:16}}>این رکورد برگشت‌پذیر نیست.</div>
            <div style={{display:"flex",gap:8,justifyContent:"center"}}>
              <button onClick={()=>setDelSub_(null)} style={{...btnG,padding:"8px 18px"}}>انصراف</button>
              <button onClick={doDelSub} style={{...btnD,padding:"8px 18px"}}>حذف</button>
            </div>
          </div>
        </div>
      )}
