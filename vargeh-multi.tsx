...
const calcMate=(ewe,ram)=>{
  ...
  if(isPregnant(ewe)){errors.push("🚫 این میش آبستن است — جفت‌گیری مجاز نیست");blocked=true;}
  ...
  if(ageMo!==null&&ageMo<12){errors.push(`🚫 سن ${toP(ageMo)} ماه — حداقل ۱۲ ماه لازم`);blocked=true;}
  ...
  if(daysSince!==null&&daysSince<45){warns.push(`⚠️ ${toP(daysSince)} روز از آخرین زایمان — حداقل ۴۵ روز`);score-=30;}
  else{oks.push("✅ فاصله زایمان مناسب");score+=15;}
  const lw=lastWeight(ewe)||0;
  if(ageMo!==null&&ageMo<18){if(lw>=45){oks.push(`✅ وزن ${toP(lw)} کیلو مناسب`);score+=15;}else{warns.push(`⚠️ وزن ${toP(lw)} کیلو پایین`);score+=5;}}
  else{if(lw>=55){oks.push(`✅ وزن ایده‌آل (${toP(lw)} کیلو)`);score+=20;}else{warns.push(`⚠️ فلاشینگ توصیه‌شده`);score+=8;}}
  if(ewe.genotype==="BB"&&ram.genotype==="BB"){score+=40;oks.push("✅ هر دو BB — بالاترین احتمال FecB");}
  ...
  if(treats.length>0){warns.push(`⚠️ ${toP(treats.length)} بیماری در ۳ ماه اخیر`);score-=15;}
...
const calcCull=a=>{
  ...
  if(sc&&sc.seasons>=2&&+sc.avg<1.3){reasons.push(`میانگین ${sc.avg} بره در ${toP(sc.seasons)} زایمان`);urgency+=3;}
  if(p<-50000000){reasons.push(`زیان‌ده: ${fmtM(p)}`);urgency+=2;}
  if(!a.hasFecBTest){reasons.push("بدون آزمایش FecB");urgency+=1;}
  if(sc&&sc.hs<40){reasons.push(`سلامت ضعیف (${toP(sc.hs)}pt)`);urgency+=2;}
  return{reasons,urgency};
};

const SAMPLE=[
  {id:"VRG-001",name:"ستاره",type:"میش",genotype:"BB",birthDate:"1402/03/15",estimatedAge:"۲ سال",entryWeight:62,entryDate:"1402/06/01",entryType:"خرید",purchasePrice:120000000,motherCode:"-",fatherCode:"-",hasFecBTest:true,hasHealthCert:true,hasVaccCard:true,
  ...
  quickNotes:[],vaccines:[{id:"v1",name:"آبله گوسفند",lastDate:"1403/07/10",nextDate:"1404/07/10",vet:"دکتر رضایی"}],
  reproductions:[{id:"r1",season:1,matingDate:"1402/07/20",birthDate:"1402/12/25",lambCount:2,birthType:"طبیعی",result:"زنده",notes:"دوقلو",ramCode:"VRG-Q01",expectedBirthDate:"1402/12/22"},{id:"r2",season:2,matingDate:"1403/07/15",result:"در انتظار",ramCode:"VRG-Q01",expectedBirthDate:"1403/12/12"}],
  weights:[{id:"w1",date:"1402/06/01",weight:62,physStatus:"ورود"},{id:"w2",date:"1403/06/01",weight:67,physStatus:"نرمال"}],
  treatments:[],status:"در گله",createdAt:new Date().toISOString()},
  {id:"VRG-Q01",name:"شیر",type:"قوچ",genotype:"BB",birthDate:"1400/05/01",estimatedAge:"۴ سال",entryWeight:85,entryDate:"1402/06/01",entryType:"خرید",purchasePrice:200000000,motherCode:"-",fatherCode:"-",hasFecBTest:true,hasHealthCert:true,hasVaccCard:true,
  costs:[{id:"c1",date:"1402/06/01",category:"خرید دام",amount:200000000,note:""}],revenues:[],quickNotes:[],vaccines:[],reproductions:[],weights:[{id:"w1",date:"1402/06/01",weight:85,physStatus:"ورود"}],treatments:[],status:"در گله",createdAt:new Date().toISOString()},
];
...
export default function VargehMulti(){
  ...
  const readBin=useCallback(async()=>{
    ...
    try{
      const res=await fetch(`https://api.jsonbin.io/v3/b/${BIN_ID}/latest`,{headers:{"X-Master-Key":MASTER_KEY,"X-Bin-Meta":"false"}});
  ...
  const writeBin=useCallback(async(data,userName)=>{
    try{
      if(!BIN_ID){
        const res=await fetch("https://api.jsonbin.io/v3/b",{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":MASTER_KEY,"X-Bin-Name":"vargeh-gelle","X-Bin-Private":"false"},body:JSON.stringify({animals:data,updatedBy:userName,updatedAt:new Date().toISOString()})});
        const j=await res.json();
        if(j.metadata?.id){BIN_ID=j.metadata.id;localStorage.setItem("vargeh_bin_id",BIN_ID);}
      }else{
        await fetch(`https://api.jsonbin.io/v3/b/${BIN_ID}`,{method:"PUT",headers:{"Content-Type":"application/json","X-Master-Key":MASTER_KEY},body:JSON.stringify({animals:data,updatedBy:userName,updatedAt:new Date().toISOString()})});
      }
    }catch(e){throw e;}
  },[]);
  ...
  const syncFromCloud=useCallback(async()=>{
    ...
    try{setSyncing(true);setSyncErr("");const rec=await readBin();if(rec?.animals){setAnimals(rec.animals);localStorage.setItem("vargeh_animals",JSON.stringify(rec.animals));setLastSync(new Date());}}
    catch{setSyncErr("خطا در دریافت");}finally{setSyncing(false);}
  },[user,readBin]);
  ...
  const saveAnimals=useCallback(async(newA)=>{
    setAnimals(newA);
    localStorage.setItem("vargeh_animals",JSON.stringify(newA));
    if(user){
      try{setSyncing(true);setSyncErr("");await writeBin(newA,user.name);setLastSync(new Date());}
      catch{setSyncErr("خطا در ذخیره ابری — ذخیره محلی انجام شد");}
      finally{setSyncing(false);}
    }
  },[user,writeBin]);

  const doLogin=()=>{
    const u=USERS.find(u=>u.nameEn===loginName&&u.pass===loginPass);
    if(!u){setLoginErr("نام کاربری یا رمز اشتباه است");return;}
    setUser(u);localStorage.setItem("vargeh_user",JSON.stringify(u));setLoginErr("");
  };
  ...
  const addSub=(aid,field,rec)=>{
    const na=animals.map(a=>{if(a.id!==aid)return a;const arr=a[field]||[];const final=field==="reproductions"?{...rec,season:arr.length+1}:rec;return{...a,[field]:[...arr,final]};});
    saveAnimals(na);setAddModal(null);toast_("✓ ثبت شد");
  };
  ...
  if(!user)return(
    <div style={{fontFamily:"'Vazirmatn',sans-serif",direction:"rtl",background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{...cs,width:"100%",maxWidth:340,padding:32}}>
        ...
        <button onClick={doLogin} style={{...btnP,width:"100%",padding:"12px",fontSize:15,justifyContent:"center"}}>ورود به سیستم</button>
      </div>
    </div>
  );

  const SubForm=({type,aid,onClose})=>{
    const[fd,setFd]=useState({});
    const sv=(k,v)=>setFd(p=>({...p,[k]:v}));
    const rs=rams;
    useEffect(()=>{if(type==="repro"&&fd.matingDate&&isDate(fd.matingDate))setFd(p=>({...p,expDate:addDays(fd.matingDate,GD)}));},[fd.matingDate,type]);
    const F=(l,k,opts,ph,tp)=>(
      <div style={{marginBottom:10}}>
        <label style={lbl}>{l}</label>
        {opts?(<select value={fd[k]||""} onChange={e=>sv(k,e.target.value)} style={inpS}><option value="">—</option>{opts.map(o=><option key={o}>{o}</option>)}</select>)
        :(<input type={tp||"text"} value={fd[k]||""} placeholder={ph||""} onChange={e=>sv(k,e.target.value)} style={inpS}/>)}
      </div>
    );
    ...
    return(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:800,padding:14}}>
        <div style={{...cs,width:"100%",maxWidth:420,maxHeight:"88vh",overflowY:"auto"}}>
          ...
          {type==="vaccine"&&<>{F("نوع واکسن","name",["آبله گوسفند","آنتروتوکسمی","بروسلوز","تب برفکی","پاستورلوز"])}{F("تاریخ تزریق","lastDate",null,"۱۴۰۳/۰۷/۱۰")}{F("تکرار بعدی","nextDate",null,"۱۴۰۴/۰۷/۱۰")}{F("دامپزشک","vet")}</>}
          ...
          {type==="repro"&&<>
            ...
            {rs.length>0&&F("قوچ","ramCode",rs.map(r=>r.id+(r.name?` — ${r.name}`:"")))}
            {fd.expDate&&<div style={{background:C.goldDim,borderRadius:8,padding:"8px 12px",marginBottom:10}}>
              <div style={{fontSize:11,color:C.gold,fontWeight:700}}>🗓️ زایمان احتمالی</div>
              <div style={{fontWeight:800,fontSize:16}}>{fd.expDate}</div>
            </div>}
          ...
          {type==="cost"&&<>
            ...
            <div style={{marginBottom:10}}>
              <label style={lbl}>مبلغ (ریال)</label>
              <input type="number" value={fd.amount||""} onChange={e=>sv("amount",e.target.value)} placeholder="مثال: 5000000" style={inpS}/>
          ...
          {type==="rev"&&<>
            ...
            <div style={{marginBottom:10}}>
              <label style={lbl}>مبلغ (ریال)</label>
              <input type="number" value={fd.amount||""} onChange={e=>sv("amount",e.target.value)} placeholder="مثال: 18000000" style={inpS}/>
              {fd.amount&&<div style={{fontSize:11,color:C.gold,marginTop:4,fontWeight:600}}>= {fmtRial(+fd.amount)}</div>}
            </div>
            {F("توضیح","note")}
          </>}
          {type==="note"&&<><div style={{marginBottom:10}}><label style={lbl}>یادداشت</label><textarea value={fd.note||""} onChange={e=>sv("note",e.target.value)} rows={3} style={{...inpS,resize:"vertical"}}/></div>{F("تاریخ","date")}</>}
          <div style={{display:"flex",gap:8,marginTop:14}}>
            <button onClick={onClose} style={{...btnG,flex:1}}>انصراف</button>
            <button onClick={()=>addSub(aid,fieldMap[type],{...fd,id:uid(),weight:fd.weight?+fd.weight:undefined,lambCount:fd.lambCount?+fd.lambCount:undefined,amount:fd.amount?+fd.amount:undefined,expectedBirthDate:fd.expDate,date:fd.date||new Date().toLocaleDateString("fa-IR")})} style={{...btnP,flex:2}}>💾 ثبت</button>
          </div>
        </div>
      </div>
    );
  ...
  const AddAnimal=({onClose})=>{
    ...
    const sv=(k,v)=>setFd(p=>({...p,[k]:v}));
    const F=(l,k,opts,ph,tp)=>(<div><label style={lbl}>{l}</label>{opts?<select value={fd[k]||""} onChange={e=>sv(k,e.target.value)} style={inpS}><option value="">—</option>{opts.map(o=><option key={o}>{o}</option>)}</select>:<input type={tp||"text"} value={fd[k]||""} placeholder={ph||""} onChange={e=>sv(k,e.target.value)} style={inpS}/> }</div>);
    const save=()=>{
    ...
    return(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:900,padding:14}}>
        <div style={{...cs,width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto"}}>
          ...
          <div style={{background:C.goldDim,borderRadius:8,padding:"10px 14px",margin:"14px 0",fontSize:12,color:C.textMid}}>💡 کد پدر را حتماً وارد کنید — برای بررسی همخونی ضروری است</div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={onClose} style={{...btnG,flex:1,padding:12}}>انصراف</button>
            <button onClick={save} style={{...btnP,flex:2,padding:12,fontSize:14}}>💾 ثبت دام</button>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{fontFamily:"'Vazirmatn',sans-serif",direction:"rtl",background:C.bg,minHeight:"100vh",color:C.text,fontSize:14,paddingBottom:62}}>
      ...
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 12px",height:54,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:200}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {syncing&&<span style={{fontSize:10,color:C.gold,fontWeight:700}}>⏳ همگام</span>}
          {syncErr&&<span style={{fontSize:10,color:C.danger}}>⚠️ آفلاین</span>}
          {lastSync&&!syncing&&!syncErr&&<span style={{fontSize:9,color:C.accent}}>✓ همگام {lastSync.toLocaleTimeString("fa-IR",{hour:"2-digit",minute:"2-digit"})}</span>}
        </div>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <div style={{background:user.color+"22",color:user.color,border:`1px solid ${user.color}44`,borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:700}}>{user.name.split(" ")[0]} — {user.role}</div>
          {page==="list"&&<button onClick={()=>setAddModal("animal")} style={{...btnP,padding:"5px 10px"}}>+ ثبت</button>}
      ...
      {syncErr&&<div style={{background:C.dangerDim,borderBottom:`1px solid ${C.danger}44`,padding:"6px 14px",fontSize:11,color:C.danger,textAlign:"center"}}>⚠️ {syncErr} — داده‌های ذخیره‌شده محلی نمایش داده می‌شود</div>}
        ...
        <div style={{padding:14,maxWidth:960,margin:"0 auto"}}>
          ...
          {vaccAlerts.filter(v=>v.dl<=0).length>0&&(
            <div style={{...cs,marginBottom:12,borderColor:C.danger+"66",borderWidth:2}}>
              <div style={{fontWeight:800,color:C.danger,marginBottom:8,fontSize:14}}>🚨 واکسن‌های سررسیده</div>
          ...
          {birthCal.filter(e=>e.dl!==null&&e.dl<=14).length>0&&(
            <div style={{...cs,marginBottom:12,borderColor:C.gold+"55"}}>
              <div style={{fontWeight:700,color:C.gold,marginBottom:8,fontSize:13}}>🐣 زایمان نزدیک</div>
              {birthCal.filter(e=>e.dl!==null&&e.dl<=14).map(e=>(
                <div key={e.aid+e.s} onClick={()=>{setSelId(e.aid);setPage("detail");}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}>
          ...
          <div style={{...cs,borderColor:C.purple+"33"}}>
            <div style={{fontWeight:700,color:C.purple,marginBottom:10,fontSize:13}}>🧬 امتیاز ژنتیکی</div>
      ...
      {page==="cal"&&(
        <div style={{padding:14,maxWidth:960,margin:"0 auto"}}>
          ...
          {birthCal.length===0&&<div style={{...cs,textAlign:"center",padding:40,color:C.textDim}}>هیچ میش آبستنی ثبت نشده</div>}
          {birthCal.map(e=>(
      ...
      {page==="detail"&&sel&&(()=>{
        ...
        return(
          <div style={{padding:14,maxWidth:960,margin:"0 auto"}}>
                    ...
                    {editName?.id===a.id?(
                      <div style={{display:"flex",gap:6,marginBottom:4}}>
                        <input value={editName.value} onChange={e=>setEditName(p=>({...p,value:e.target.value}))}
                          ...
                          style={{background:C.surface,border:`1px solid ${C.accent}`,borderRadius:6,padding:"4px 8px",color:C.text,fontFamily:"inherit",fontSize:16,fontWeight:800,width:140}} autoFocus/>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4}}>
                        <div style={{fontSize:17,fontWeight:800}}>{a.name||a.id}</div>
                        <button onClick={()=>setEditName({id:a.id,value:a.name||""})} style={{...btnG,padding:"2px 8px",fontSize:11}}>✏️ نام</button>
                      </div>
                    )}
                    <div style={{fontSize:12,color:C.textMid}}>{a.id} · {a.type} · {a.estimatedAge||"—"}</div>
            ...
            {subMap[dtab]&&<div style={{marginBottom:10}}><button onClick={()=>setAddModal(subMap[dtab])} style={{...btnP,padding:"6px 12px"}}>+ افزودن {tl[dtab]}</button></div>}
            {dtab==="info"&&<div style={cs}><div style={g3}>
              {[["کد",a.id],["نوع",a.type],["ژنوتیپ",a.genotype],["تولد",a.birthDate],["سن",a.estimatedAge],["وزن ورود",a.entryWeight?`${toP(a.entryWeight)} کیلو`:null],["کد پدر",a.fatherCode||"—"],["کد مادر",a.motherCode||"—"]].map(([l,v])=>(
            ...
            {dtab==="vacc"&&<div style={cs}>
              ...
              {(a.vaccines||[]).map(v=>(<div key={v.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  ...
                  <button onClick={()=>delSub(a.id,"vaccines",v.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:18}}>🗑</button>
                </div>
                <div style={{fontSize:11,color:C.textDim}}>{v.lastDate} ← {v.nextDate}</div>
              </div>))}
            </div>}
              ...
              {(a.reproductions||[]).map(r=>(<div key={r.id} style={{...cs,borderColor:r.result==="در انتظار"?C.gold+"44":C.border}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                  ...
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    ...
                    <button onClick={()=>delSub(a.id,"reproductions",r.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:18}}>🗑</button>
                  </div>
                </div>
                <div style={g3}>
            ...
            {dtab==="weight"&&<div style={cs}>
              ...
              {(a.weights||[]).map(w=>(<div key={w.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                ...
                <button onClick={()=>delSub(a.id,"weights",w.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:18}}>🗑</button>
              </div>))}
            </div>}
            {dtab==="treat"&&<div style={cs}>
              ...
              {(a.treatments||[]).map(t=>(<div key={t.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  ...
                  <button onClick={()=>delSub(a.id,"treatments",t.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:18}}>🗑</button>
                </div>
            ...
            {dtab==="cost"&&<div style={cs}>
              ...
              {(a.costs||[]).map(c=>(<div key={c.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                <div><div style={{fontWeight:600}}>{c.category}</div><div style={{fontSize:11,color:C.textDim}}>{c.date}{c.note?` · ${c.note}`:""}</div></div>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <span style={{fontWeight:700,color:C.danger,fontSize:12}}>{fmtRial(+c.amount)}</span>
                  <button onClick={()=>delSub(a.id,"costs",c.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:18}}>🗑</button>
                </div>
              </div>))}
            ...
            {dtab==="rev"&&<div style={cs}>
              ...
              {(a.revenues||[]).map(r=>(<div key={r.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                <div><div style={{fontWeight:600}}>{r.category}</div><div style={{fontSize:11,color:C.textDim}}>{r.date}{r.note?` · ${r.note}`:""}</div></div>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <span style={{fontWeight:700,color:C.accent,fontSize:12}}>{fmtRial(+r.amount)}</span>
                  <button onClick={()=>delSub(a.id,"revenues",r.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:18}}>🗑</button>
                </div>
              </div>))}
            ...
            {dtab==="note"&&<div style={cs}>
              ...
              {(a.quickNotes||[]).slice().reverse().map(n=>(<div key={n.id} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  ...
                  <button onClick={()=>delSub(a.id,"quickNotes",n.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:18}}>🗑</button>
                </div>
              </div>))}
            </div>}
          </div>
      ...
      {page==="ai"&&(()=>{
        ...
        const vaccPlan=[{name:"تب برفکی",season:"فروردین و مهر",freq:"۲ بار",target:"تمام گله",req:true,law:true},{name:"بروسلوز",season:"پاییز",freq:"یک‌بار عمر",target:"میش نابالغ",req:true,law:true},{name:"آنتروتوکسمی",season:"۴ هفته قبل زایمان",freq:"سالانه",target:"میش آبستن",req:true,law:false},{name:"آبله گوسفند",season:"مهر",freq:"سالانه",target:"تمام گله",req:true,law:false},{name:"پاستورلوز",season:"پاییز",freq:"سالانه",target:"تمام گله",req:false,law:false}];
        ...
        return(
          <div style={{padding:14,maxWidth:960,margin:"0 auto"}}>
            ...
            {analysisTab==="mate"&&mateResults.map(({ewe,rams:ramResults,sc})=>(
            ...
            {analysisTab==="vacc"&&<div>
              {overdueV.length>0&&<div style={{...cs,marginBottom:12,borderColor:C.danger+"66",borderWidth:2}}>
              ...
              {vaccPlan.map((v,i)=>(
                <div key={i} style={{...cs,marginBottom:8,borderColor:v.law?C.danger+"44":v.req?C.accent+"33":C.border}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
                    <div style={{fontWeight:700}}>{v.name}
                      {v.law&&<Bdg l="⚡ قانونی" c={C.danger} sm/>}
